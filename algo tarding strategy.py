...
ma_1 = 30 #30 day moving avarage
ma_2 = 100 # 100 day moovintg avarage

start = dt.datetime.now() - dt.timedelta(days=365 *3) #3y dispaly of data
end = dt.datetime.now()

data = web.DataReader('MSFT','yahoo',start, end) #get the stock data from yahoo finance api
#adding mooving avarage for first and second variable
data[f'SMA_{ma_1}'] = data['Adj Close'].rolling(window=ma_1).mean() 
data[f'SMA_{ma_2}'] = data['Adj Close'].rolling(window=ma_2).mean()

# visualising the data itself starting from 100day mooving avarage 
data = data.iloc[ma_2:]
